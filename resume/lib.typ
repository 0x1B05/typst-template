...
#let icon(path, fill: rgb("#000000")) = box(
  ...
  align(
    ...
    image(
      bytes(
        read(path).replace("path d", "path fill=\"" + fill.to-hex() + "\" d"),
      ),
      height: 1em,
    ),
  ),
...
#let resume(
  ...
  if header-center {
    assert(
      photograph == "",
      message: "can not centerize the name with the photo",
    )
    ...
    grid(
      ...
      [#header #introduction],
      if photograph != none {
        set image(width: photograph-width)
        align(right, photograph)
      },
    )
  }
